...
const ParentSelection = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-pink-50 flex flex-col items-center justify-center p-4 sm:p-4 md:p-5 lg:p-6">
      {/* Header */}
      <motion.div
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        className="text-center mb-8 sm:mb-8 md:mb-10 lg:mb-12">
        <h1 className="text-3xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-gray-800 mb-3">
          Hi, {parentName}! 👋
        </h1>
        <p className="text-base sm:text-base md:text-lg lg:text-xl text-gray-500">
          What would you like to do with your child today?
        </p>
      </motion.div>

      {/* Options */}
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-5 sm:gap-5 md:gap-6 lg:gap-8 max-w-3xl w-full">
        {options.map((opt, idx) => (
          <motion.div
            key={idx}
            initial={{ opacity: 0, y: 30 }}
            ...
            whileTap={{ scale: 0.97 }}
            onClick={() => navigate(opt.path)}
            className={`cursor-pointer bg-gradient-to-br ${opt.color} p-6 sm:p-6 md:p-7 lg:p-8 rounded-[32px] shadow-2xl text-white relative overflow-hidden group`}
          >
            {/* Background emoji */}
            <div className="absolute -right-4 -top-4 text-8xl opacity-20
              group-hover:opacity-30 group-hover:rotate-12 transition-all duration-300">
            ...
            {/* Icon */}
            <div className="bg-white/20 w-16 h-16 sm:w-16 sm:h-16 md:w-18 md:h-18 lg:w-20 lg:h-20 rounded-3xl flex items-center
              justify-center mb-4 sm:mb-4 md:mb-5 lg:mb-6 group-hover:bg-white/30 transition-colors">
              {opt.icon}
            </div>
            {/* Text */}
            <h2 className="text-2xl sm:text-2xl md:text-2xl lg:text-3xl font-bold mb-2">{opt.title}</h2>
            <p className="text-white/80 text-base sm:text-base md:text-base lg:text-lg leading-snug">{opt.desc}</p>
            {/* Arrow */}
            <div className="mt-4 sm:mt-4 md:mt-5 lg:mt-6 flex items-center gap-2 text-white/70 font-semibold">
              <span>Let's Go</span>
              <span className="text-xl group-hover:translate-x-2 transition-transform">→</span>
            </div>
          </motion.div>
      ...
      <motion.button
        ...
        transition={{ delay: 0.4 }}
        onClick={() => navigate('/parent/home')}
        className="mt-8 sm:mt-8 md:mt-9 lg:mt-10 text-gray-400 hover:text-gray-600 text-sm font-semibold transition-colors">
        ← Go to Parent Portal
      </motion.button>
    </div>
  );
