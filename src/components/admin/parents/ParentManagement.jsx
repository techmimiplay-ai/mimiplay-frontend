...
const ParentManagement = () => {
  ...
  const handleApprove = async (parent) => {
    await fetch(`${API_BASE_URL}/api/admin/approve/${parent.id}`, { method: "PUT" });
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-text mb-2">Parent Management</h1>
        <p className="text-text/60">Manage parent accounts and approvals</p>
      </div>
          ...
          <table className="w-full min-w-[600px]">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left py-4 px-4 font-semibold text-text">Parent</th>
                <th className="text-left py-4 px-4 font-semibold text-text">Contact</th>
                <th className="text-left py-4 px-4 font-semibold text-text hidden sm:table-cell">Child</th>
                ...
                >
                  ...
                  <td className="py-4 px-4">
                    <div className="space-y-1">
                      <p className="text-sm text-text flex items-center gap-1">
                        ...
                        <span className="truncate max-w-[120px]">{parent.email}</span>
                      </p>
                      <p className="text-sm text-text flex items-center gap-1">
                  ...
                  <td className="py-4 px-4 hidden sm:table-cell">
                    {(() => {
                      ...
                      return parentChildren.length === 0 ? (
                        ...
                        parentChildren.map((child) => (
                          <p key={child._id} className="font-semibold text-text">{child.name}</p>
                        ))
                      );
                    })()}
                  </td>
                  <td className="py-4 px-4 text-text/70 hidden md:table-cell">
                    {(() => {
                      const parentChildren = getChildrenOfParent(parent.id);
                      return parentChildren.length === 0 ? (
                        <p className="text-gray-400">-</p>
                        ...
                        parentChildren.map(child => (
                          <p key={child._id} className="text-sm">{child.class}</p>
                        ))
                      );
                    })()}
                  </td>
                  <td className="py-4 px-4">
                    <span className={`
                      ...
                      ${parent.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}
                    `}>
                      {parent.status === 'active' && <CheckCircle size={12} />}
                      {parent.status.charAt(0).toUpperCase() + parent.status.slice(1)}
                    </span>
                  </td>
                  <td className="py-4 px-4">
                    <div className="flex items-center justify-end gap-1">
                      <button
                        onClick={() => { setSelectedParent(parent); setShowApprovalModal(true); }}
      ...
      {/* View/Approval Modal */}
      {selectedParent && (
        <Modal isOpen={showApprovalModal} onClose={() => setShowApprovalModal(false)} title="Parent Details" size="md">
          <div className="space-y-4">
            <div className="flex items-center gap-4 pb-4 border-b">
              <Avatar size="xl" />
              <div>
                <h3 className="text-xl sm:text-2xl font-bold text-text">{selectedParent.name}</h3>
                <span className={`
                  inline-block px-3 py-1 rounded-full text-sm font-semibold mt-1
                  ${selectedParent.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}
                `}>
                  {selectedParent.status.charAt(0).toUpperCase() + selectedParent.status.slice(1)}
                </span>
              </div>
            ...
            <div className="space-y-3 bg-gray-50 rounded-2xl p-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-text truncate">{selectedParent.email}</span>
              </div>
              <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                <User size={18} className="text-text/60 shrink-0" />
                <span className="text-text">Child: {getChildrenOfParent(selectedParent.id).map(child => (
                  <p key={child._id}>{child.name}</p>
                ))}</span>
              </div>
      ...
      {/* Edit Modal */}
      {editForm && (
        <Modal isOpen={showEditModal} onClose={() => setShowEditModal(false)} title="Edit Parent" size="md">
          <div className="space-y-4">
            ...
            <Input label="Child Class" value={editForm.childClass}
              onChange={(e) => setEditForm({ ...editForm, childClass: e.target.value })} />
            <select
              className="w-full border rounded-lg p-2"
              value={editForm.status}
