...
const ActivityLog = ({ selectedChild }) => {
  ...
  const filteredResults = allResults.filter(r => {
    ...
    if (appliedType !== 'all' && !(r.activityName || '').toLowerCase().includes(appliedType)) return false;
    return true;
  });
  ...
  const avgScore        = totalActivities > 0 ? (totalStars / totalActivities).toFixed(1) : '—';

  const downloadLog = () => {
    if (filteredResults.length === 0) { toast('No activities to download.', 'warning'); return; }
    const csv = [
  ...
  if (!selectedChild) return <EmptyState icon={BookOpen} title="No child selected" description="Please select a child from the top menu" />;
  if (loading) return <PageLoader variant="inline" emoji="📚" text="Loading activity log…" />;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200 text-center">
          <div className="text-3xl mb-2">📅</div>
          <p className="text-3xl font-bold text-purple-900">{Object.keys(grouped).length}</p>
      ...
      {groupedDays.length === 0 && (
        <EmptyState
          ...
          description={appliedDate !== 'all' || appliedType !== 'all' ? 'Try changing the filters' : 'No activities completed yet'}
        />
      )}
            ...
            <div className="space-y-3">
              {dayData.activities.map((activity, actIndex) => (
                <motion.div key={activity.id || actIndex}
                  ...
                  className="flex flex-col md:flex-row items-start gap-3 md:gap-4 p-4 bg-gray-50 rounded-2xl hover:bg-gray-100 transition-colors">
                  ...
                  <div className="flex-1 w-full">
                    <div className="flex flex-col md:flex-row items-start justify-between gap-2 mb-2">
                      <div>
                        <h3 className="font-semibold text-text text-lg">{activity.activityName || 'Activity'}</h3>
                        <span className={`inline-block px-3 py-1 rounded-lg text-xs font-semibold mt-1 ${getTypeColor(activity.activityName)}`}>
