...
const SettingsTab = () => {
  ...
  const fetchProfile = async () => {
    try {
      setLoading(true);
      const res = await apiRequest('get', API_ENDPOINTS.PARENT_PROFILE(parentId));
  ...
  const handleSave = async () => {
    try {
      ...
      await apiRequest('put', API_ENDPOINTS.PARENT_PROFILE(parentId), {
        name:       formData.name,
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-text mb-2">Settings ⚙️</h1>
      ...
      {/* Profile Settings */}
      <Card>
        <div className="flex flex-col sm:flex-col md:flex-row lg:flex-row items-start md:items-center gap-4 mb-8">
          ...
          <div>
            <h3 className="text-xl sm:text-xl md:text-2xl lg:text-2xl font-bold text-text">{formData.name || 'Parent'}</h3>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            <Button variant="primary" className="flex-1" disabled={pwSaving} onClick={async () => {
              ...
              setPwSaving(true);
              try {
                await apiRequest('put', API_ENDPOINTS.PARENT_CHANGE_PASSWORD(parentId), {
