...
const AchievementsTab = ({ selectedChild }) => {
  ...
  const fetchConfig = async () => {
    try {
      // Fetch badges config
      const badgesRes = await axios.get(`${API_BASE_URL}/api/config/badges`);
      if (badgesRes.data?.status === 'success') {
        setAllBadges(badgesRes.data.badges);
      }

      // Fetch leaderboard
      const leaderboardRes = await axios.get(`${API_BASE_URL}/api/parent/class-leaderboard`);
      if (leaderboardRes.data?.status === 'success') {
        const lbData = leaderboardRes.data.leaderboard.map((item, i) => ({
          ...item,
          rank: i + 1,
          isCurrentUser: item.student_id === selectedChild?.id
        }));
  ...
  const earned     = allBadges.filter(b => totalStars >= b.unlockAt);
      ...
      <Card className="bg-gradient-to-r from-yellow-400 to-orange-400 border-0 text-white text-center py-6">
        <motion.div key={totalStars} initial={{ scale: 0.8 }} animate={{ scale: 1 }}
          ...
          <p className="text-white/70 text-sm mt-1">
            {earned.length} / {allBadges.length} badges unlocked
          </p>
        </motion.div>
      </Card>
