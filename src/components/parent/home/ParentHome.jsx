...
const ParentHome = ({ selectedChild }) => {
  ...
  // Derive achievements from real activity data
  const achievements = (() => {
    const earned = [];
    if (totalStars >= 1) earned.push({ icon: '🌟', title: 'First Star', description: 'Earned first star!' });
    if (todayCount >= 3) earned.push({ icon: '🔥', title: 'Active Day', description: '3+ activities today' });
    if (todayResults.length > 0 && todayResults.every(r => r.stars === 5))
      earned.push({ icon: '💯', title: 'Perfect Score', description: 'All 5-star today!' });
    if (totalStars >= 50) earned.push({ icon: '🏆', title: 'Champion', description: '50 stars earned' });
    if (allResults.length >= 10) earned.push({ icon: '📚', title: 'Bookworm', description: '10 activities done' });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        >
          ...
          <div>
            <p className="font-bold text-lg leading-tight">Chat with Alexi</p>
            <p className="text-white/80 text-sm">AI-powered learning chat 🤖</p>
          </div>
        ...
        >
          ...
          <div>
            <p className="font-bold text-lg leading-tight">Learning Activities</p>
            <p className="text-white/80 text-sm">Fun activities at home 🎮</p>
          </div>
          <span className="ml-auto text-2xl opacity-60">→</span>
        </motion.button>
      </div>
            ...
            <p className="text-3xl sm:text-4xl font-bold text-purple-900">
              {(() => {
                ...
                while (true) {
                  ...
                  if (allResults.some(r => r.date === ds)) { streak++; d.setDate(d.getDate() - 1); }
                  else break;
                }
                return streak;
              })()}
      ...
      <Card>
        <h2 className="text-xl sm:text-2xl font-bold text-text mb-4">🏆 Recent Achievements</h2>
